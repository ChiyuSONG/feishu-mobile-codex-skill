...
Provide a small, understandable, DIY-friendly mobile entrance to local Codex. Automate setup where practical without presenting it as fully unattended: account state, permission approval, CAPTCHA, or platform differences may still require brief user participation. Keep normal use conversational, and never expose internal project keys, thread IDs, credentials, or private paths unless diagnosis requires them.
...
Before changing the machine, say briefly what will happen, that the user may need to scan a Feishu QR code, and that account, permission, or platform differences can require a few guided manual steps. Also give the first-run permission notice: the default is Codex `Full Access` (`danger-full-access` with no interactive approvals), which lets the local Codex process read, write, and run tools across the machine. Explain that task instructions still bind each group to its registered project and retain the safety gates below, but a malicious or mistaken instruction has a larger blast radius. Tell the user they can say “改成仅项目权限” / “use project-only access” or “改回逐次审批” / “use auto review” before or after setup. New remote profiles default to Full Access independently of the source task sandbox; disclose this before setup and honor an explicit user downgrade. Preserve existing remote profiles. For creating the patrol task, use `patrol_creation_settings(project)`: pass its supported model/reasoning fields to the task tool and verify Priority/permissions in the client; disclose fields the tool cannot apply. Then follow [setup.md](references/setup.md). Concentrate unavoidable human actions—QR login, consent, CAPTCHA, required app-permission approval, tenant-admin approval, or secure secret entry—at the beginning. Before CUA clicks any create-app control, it must classify the currently selected Feishu account. Default to and recommend a personal account for first-time setup. If the account is team/company/enterprise, or its type is unclear, stop before app creation, explain the administrator-approval and organizational-visibility implications, and ask the user to switch to a personal account. Continue in a team account only when the user's stated goal explicitly targets a group under that tenant and the user gives informed consent for that specific tenant; silence, prior broad automation permission, or inability to classify the account is not consent. Treat native image/file upload permission as an installation prerequisite, not a repair suggestion after delivery fails. Do not announce a successful connection until a real image has been received in the bound Feishu group.

## Reuse The Feishu Application First

After the account-type gate passes, inventory known user-level gateway and publisher registries, installed Listener configurations, protected credential metadata, and self-built applications in the approved tenant. Verify tenant, ownership, bot and long-connection capability, document delivery, required scopes, credential availability, and Listener topology. A new project or group is not a reason to create a new app.
...
- Let the user send consecutive messages without waiting. Follow the single queue interaction contract in [architecture.md](references/architecture.md); do not duplicate selection rules in prompts.
...
- Leave messages in Feishu while the computer is offline. Reconcile them at Listener startup, immediately after a confirmed reconnect, and through the existing hourly inspection for workstreams where that inspection is enabled.
...
- Do not accept native image delivery from a local-file check, document fallback, or mocked upload. Verify `im:resource` (or the current upload equivalent) and complete one real persisted-thread image reply visible in Feishu before setup or repair is called successful.
...
- Present the bundled behavior as a useful default rather than a fixed ceiling. Let users request changes or extensions in natural language, including separate behavior for different registered groups. Concise examples may include a scheduled industry-news digest, handling colleague questions after an explicit company-group setup, or different reply and inspection rules per project group. Label these as DIY examples rather than bundled defaults. Before binding a multi-member work group, explain that member messages delivered to the bot may invoke local Codex, verify the intended member scope, and agree on the local permission profile. Implement and verify customization only inside each group's bound project, permission profile, privacy boundary, and applicable safety rules; never promise that every requested extension is automatically possible.
...
For projects sharing one local gateway and Feishu bot, `init-project` persists `inspection_report_mode`: the first registration defaults to `full`; additional new registrations default to `status`. Preserve existing modes and customizations; legacy registrations without this field retain their full-report behavior. Do not rewrite existing Automations or move reminders during registration. Users can change the mode through conversation by updating the selected project's configuration; these are defaults, not restrictions.

In `full` mode, send the formal first-inspection explanation exactly once, then three short lines: current Listener/message state, Token plan usage, and a tip about customization through chat instructions. In `status` mode, send only the status line from the very first inspection, including when idle; do not query usage or send the first-inspection explanation or customization tip. When a serialized batch is `processing`, the status line must also include its elapsed time and latest safe user-visible Codex progress. Read only the durable state and current JSONL event tail; never expose the source prompt, commands, raw output, hidden reasoning, internal IDs, or local paths. A separately customized inspection with the default report disabled retains the existing progress-only behavior while processing. Deduplicate progress by project, active batch, and local hour. Keep reminders off until requested and scoped to the conversation the user selects; never copy reminders or DIY behavior into additional projects. If the user changes the inspection through conversation, honor the new content instead of restoring the default.
...
Install the user-level Codex `SessionStart` Hook from `scripts/install_startup_hook.py` after the Listener launcher exists. Match `startup|resume` and call the idempotent Listener start command, so opening or resuming a Codex session does not wait for the hourly inspection. Never bypass Codex Hook trust review, overwrite unrelated hooks, or describe this as OS login startup. Keep the hourly inspection for reports, reminders, and its existing catch-up fallback.
...
- Provision new profiles through `scripts/project_profiles.py`: Desktop (A) stays user-controlled; patrol (B) takes `PATROL_DEFAULTS`; remote Feishu (C) inherits the explicitly selected source task's model, reasoning and Priority once and is then pinned independently; new remote permissions default to `full-access`, independently of the source task sandbox. Preserve existing explicit B/C fields. Never fill missing C fields from B defaults or continuously follow mutable Desktop settings. Pass `--source-thread-id` or the confirmed `--source-model`, `--source-reasoning-effort` and `--source-service-tier` values to `init-project`. Missing model/reasoning/tier metadata requires confirmation; permission does not require source metadata. Use `--source-permission-mode` or `--permission-mode` for an explicit user override, and preserve existing explicit lower modes. General workstreams can supply explicitly confirmed values without scanning unrelated tasks. Priority is the internal term; Fast is its legacy alias, not a second speed option. This does not authorize a separate API billing lifecycle.
- Use the disclosed new-remote permission default above; preserve existing explicit settings. The missing-permission fallback remains `full-access`, implemented as `--sandbox danger-full-access` plus `approval_policy="never"`. This prevents routine local tools, installed CLIs, and long-running work from stopping for Codex approvals. Keep `project-only-auto` (`workspace-write` with normal network access) and `auto-review` available as conversational downgrade modes. Full Access is a technical capability, not permission to cross the registered project scope.
...
- Verify the selected Feishu tenant before creating chats, sending messages, or publishing documents. Default to a personal account; never create an app in a team/company tenant without the explicit informed opt-in required above.
...
For repairs or cross-edition fix ports, read [runtime-recovery.md](references/runtime-recovery.md) for native capacity handling, original-task recovery, durable failure/completion notices, history-fork compatibility and executable acceptance scenarios. Keep domain criteria and project records outside this package.
