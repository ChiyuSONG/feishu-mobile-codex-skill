...
安装时建议使用 **GPT-6 Astra**，并**强烈建议开启 Full Access**；新项目连接后也默认使用这套配置。推理等级和快速模式（Fast mode）按需选择，远程任务沿用本机配置。已有项目的显式模型、推理和加速设置保留，各项目可通过对话指令单独调整。
...
- `把这份销售表按产品汇总，找出销售额下降最多的几项，生成 Excel 发给我。`
- `每周五下午 5 点提醒我交周报，直到我说已经提交。`

你可以在同一个飞书群里连续发消息，Codex 会结合上下文，自动识别哪些是同一任务的补充并合并处理，不同任务则排队串行处理。消息开头加 `*` 可强制单独处理，不自动与其他消息合并；加 `#` 可继承当前对话上下文，开启独立上下文并立即并行处理。默认每小时自动巡检连接和任务状态。电脑或 Codex 关闭时不会执行任务，但暂存在飞书的消息不会丢失，下次打开 Codex 后会自动继续处理。
...
不会。简单回复直接发在群里；较长或结构复杂的内容才使用私有飞书文档。图片或附件发送失败时会明确提示。
