...
- Reply directly for simple results. Send explicitly linked image artifacts as native Feishu image replies so they are immediately visible; use the bundled private Feishu publisher for long or structured results and other artifacts, then send a concise group summary and link. If publication, image delivery, or privacy hardening fails, keep the text result and state the artifact failure instead of silently dropping it.
