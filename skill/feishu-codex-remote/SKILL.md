...
- Provision new profiles through `scripts/project_profiles.py`: Desktop (A) stays user-controlled; patrol (B) takes `PATROL_DEFAULTS`; remote Feishu (C) inherits the explicitly selected source task once and is then pinned independently. Preserve existing explicit B/C fields. Never fill missing C fields from B defaults or continuously follow mutable Desktop settings. Pass `--source-thread-id` or the confirmed `--source-model`, `--source-reasoning-effort`, `--source-service-tier`, and `--source-permission-mode` values to `init-project`. If the exact task metadata cannot supply a field, obtain confirmation instead of guessing. General workstreams can supply explicitly confirmed values without scanning unrelated tasks. Priority is the internal term; Fast is its legacy alias, not a second speed option. This does not authorize a separate API billing lifecycle.
- Inherit new remote permissions as specified above; preserve existing explicit settings. The legacy missing-permission fallback remains `full-access`, implemented as `--sandbox danger-full-access` plus `approval_policy="never"`. This prevents routine local tools, installed CLIs, and long-running work from stopping for Codex approvals. Keep `project-only-auto` (`workspace-write` with normal network access) and `auto-review` available as conversational downgrade modes. Full Access is a technical capability, not permission to cross the registered project scope.
