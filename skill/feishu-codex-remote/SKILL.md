...
Send the formal first-inspection explanation exactly once. On later unchanged runs, send one compact Feishu message containing exactly three short lines: current Listener/message state, Token plan usage, and a reminder that natural language can change the inspection or add reminders and other uses. When a serialized batch is `processing`, the first line must also include its elapsed time and latest safe user-visible Codex progress. Read only the durable state and current JSONL event tail; never expose the source prompt, commands, raw output, hidden reasoning, internal IDs, or local paths. If the default three-line report was disabled, send a separate progress-only message only while a task is processing. Deduplicate progress by project, active batch, and local hour. Keep reminders off until requested. If the user changes the inspection through conversation, honor the new content instead of restoring the default.
